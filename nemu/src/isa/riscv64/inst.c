...
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(dest) = s->pc + 4; s->dnpc = s->pc + src1; down_ftrace(s->dnpc));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + src2) & 0xfffffffffffffffe; R(dest) = s->pc + 4; down_ftrace(s->dnpc));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if(src1 == src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, if(src1 != src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, if((int64_t)src1 >= (int64_t)src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, if((int64_t)src1 < (int64_t)src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, if(src1 < src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
  
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, if(src1 >= src2){s->dnpc = s->pc + dest; down_ftrace(s->dnpc);});
